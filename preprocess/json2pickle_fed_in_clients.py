...
for directory_name in dir_list:
    ...
    for data_name in data_names:
        ...
        for client_id in range(num_clients):
            ...
            train_data_prefix_id = train_data_prefix + "_" + str(client_id)
            for file in tqdm(all_files):
                if train_data_prefix_id in file:
            ...
            filehandler = open(output_dir + train_data_prefix_id + ".pkl", "wb")
            pickle.dump(train_data_dict_same, filehandler)
            filehandler.close()

            # print("valid")
            valid_dict_list = []
            valid_data_prefix_id = valid_data_prefix + "_" + str(client_id)
            for file in tqdm(all_files):
                if valid_data_prefix_id in file:
                    with open(directory_name + file, "r") as fin:
                        data_dict = json.load(fin)
            ...
            if len(valid_dict_list) > 0:
                ...
                filehandler = open(output_dir + valid_data_prefix_id + ".pkl", "wb")
                pickle.dump(valid_data_dict, filehandler)
                filehandler.close()

            # print("test")
            test_dict_list = []
            test_data_prefix_id = test_data_prefix + "_" + str(client_id)
            for file in tqdm(all_files):
                if test_data_prefix_id in file:
                    with open(directory_name + file, "r") as fin:
                        data_dict = json.load(fin)
            ...
            if len(test_dict_list) > 0:
                ...
                filehandler = open(output_dir + test_data_prefix_id + ".pkl", "wb")
