...
class GQE(model.IterativeModel):
    def __init__(self, num_entities, num_relations, embedding_size, label_smoothing=0.1, use_old_loss=False,negative_size=128):
        ...
        # union
        self.union_nn_layer_1 = nn.Linear(embedding_size, embedding_size // 2)
        self.union_nn_layer_2 = nn.Linear(embedding_size // 2, embedding_size)
        self.union_transformation_matrix = nn.Linear(embedding_size, embedding_size, bias=False)
    ...
    def union(self, sub_query_encoding_list):
        ...
        # [batch_size, number_sub_queries, embedding_size]
        all_subquery_encodings = self.union_nn_layer_1(all_subquery_encodings)
        all_subquery_encodings = self.relu(all_subquery_encodings)
        all_subquery_encodings = self.union_nn_layer_2(all_subquery_encodings)
        ...
        # The transformation matrix
        # [batch_size, embedding_size]
        all_subquery_encodings = self.union_transformation_matrix(all_subquery_encodings)
