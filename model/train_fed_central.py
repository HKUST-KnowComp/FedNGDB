...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_clients", type=int, default=3, choices=[3, 5])
    ...
    parser.add_argument("--server_model", type=str, choices=["NA"])
    ...
    args.checkpoint_path = args.checkpoint_path + "/fed-1"
    ...
    # client data preparation and client server generation
        ...
        # TODO read KG graph and get the entity id mapping for clients
    ...
    train_query_file_names = data_name + "_train_queries" + ".pkl"
    valid_query_file_names = [data_name + "_valid_queries_" + str(i) + ".pkl" for i in range(args.n_clients)]
    test_query_file_names = [data_name + "_test_queries_" + str(i) + ".pkl" for i in range(args.n_clients)]


    for i in range(args.n_clients):

        client_entity_mask = [1] * nentity
        client_relation_mask = [1] * nrelation
        ...
        with open(args.query_dir + "/" + train_query_file_names, "rb") as fin:
        ...
        # TODO change valid and test data to dataloader
        with open(args.query_dir + "_fed/" + data_name + "/fed-" + str(args.n_clients) + "/" + valid_query_file_names[i], "rb") as fin:
        ...
        with open(args.query_dir + "_fed/" + data_name + "/fed-" + str(args.n_clients) + "/" + test_query_file_names[i], "rb") as fin:
    ...
    for i in range(args.n_clients):
        ...
        # TODO add entity mask to client evaluation
        # TODO add update mask to client aggregation
    ...
    if args.evaluate_cross:
        # with open(args.query_dir + "_fed/" + data_name+ "/fed-" + str(args.n_clients) + "/"+ data_name + "_cross_clients_train_queries.pkl", "rb") as fin:
        #     train_data_dict = pickle.load(fin)
        # for query_type, query_answer_dict in train_data_dict.items():
        #     new_iterator = SingledirectionalOneShotIterator(DataLoader(
        #         TrainDataset(nentity, nrelation, query_answer_dict),
        ...
        with open(args.query_dir + "_fed/" + data_name+ "/fed-" + str(args.n_clients) + "/"+ data_name + "_cross_clients_valid_queries.pkl", "rb") as fin:
            valid_data_dict = pickle.load(fin)
        for query_type, query_answer_dict in valid_data_dict.items():
            new_iterator = DataLoader(
                ValidDataset(nentity, nrelation, query_answer_dict),
        ...
        with open(args.query_dir + "_fed/" + data_name+ "/fed-" + str(args.n_clients) + "/"+ data_name + "_cross_clients_test_queries.pkl", "rb") as fin:
            test_data_dict = pickle.load(fin)
        for query_type, query_answer_dict in test_data_dict.items():
            new_iterator = DataLoader(
                TestDataset(nentity, nrelation, query_answer_dict),
    ...
    for step in tqdm(range(1, args.global_steps)):
        ...
        server.aggregate_and_update(sample_id, client_data, ent_update, rel_update)
        ...
        if args.evaluate_cross and step % args.log_steps == 0:
            ...
            for task_name, loader in cross_valid_iterators.items():
                ...
                for batched_query, unified_ids, train_answers, valid_answers in loader:
                    ...
                    generalization_logs_batch = server.model.evaluate_generalization(query_embedding, train_answers,
                                                                                   valid_answers)
            ...
            for task_name, loader in cross_test_iterators.items():
                ...
                for batched_query, unified_ids, train_answers, valid_answers, test_answers in loader:
                    query_embedding = server.cross_query(batched_query)
                    generalization_logs_batch = server.model.evaluate_generalization(query_embedding, valid_answers,
                                                                                     test_answers)
